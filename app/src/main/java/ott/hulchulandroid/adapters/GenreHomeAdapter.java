...
import ott.hulchulandroid.ItemMovieActivity;
...
public class GenreHomeAdapter extends RecyclerView.Adapter<GenreHomeAdapter.OriginalViewHolder> {
    ...
    public void onBindViewHolder(GenreHomeAdapter.OriginalViewHolder holder, final int position) {
      ...
      /*
        //added new adapter for change layout if else seperate itemlist layout
        if (obj.getName().contains("Hindi")) {
            HindiPageAdapter hindiadapter = new HindiPageAdapter(ctx, obj.getList());
            holder.recyclerView.setLayoutManager(new LinearLayoutManager(ctx, LinearLayoutManager.HORIZONTAL, false));
            holder.recyclerView.setAdapter(hindiadapter);
         //   holder.recyclerView.smoothScrollToPosition(hindiadapter.getItemCount() - 1);
